...
| `network_error` | Couldn't reach one specific host (this skill talks to `www.wikidata.org`, `wikimedia.org`, and a `*.wikipedia.org` host per language -- a failure on one doesn't mean the others are blocked) | **Verify with a raw `curl` on the exact URL that failed** (the `hint` field gives you that exact command, already filled in) -- a failure in this specific process is not proof of a real block. If you're in a sandboxed Bash tool, also check the Bash tool's own result for a "host not allowed"-style message before assuming it's the user's network -- that means the sandbox hasn't approved this host, which is fixed by approving/declaring it, not by contacting IT. Real incident (twice, independently): a prior run confidently diagnosed "org network policy" here and was wrong -- the process itself (or an unapproved sandbox host) was the problem, not the user's actual network. |
