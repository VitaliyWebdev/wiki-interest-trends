            ...
            if attempt >= max_retries:
                raise AppError(
                    ...
                    hint=(
                        ...
                        "If curl also fails, the fix depends on which Claude "
                        "product you're running as -- you already know this, "
                        "so use it: in Claude Code, check the Bash tool's own "
                        "result for this command (separately from this "
                        "script's error) for a message naming a "
                        "disallowed/blocked host -- that means the sandbox "
                        "hasn't approved this host yet, fixed by approving or "
                        "declaring it, not a network problem. In claude.ai "
                        "chat or Claude Desktop, with no local Bash tool "
                        "involved, this is almost always the Capabilities > "
                        "Code execution > Allow network egress setting (or, "
                        "for Team/Enterprise, the org admin's equivalent), "
                        "not the user's actual network -- tell them to add "
                        "these hosts there or pick 'All domains', and warn "
                        "that it can still fail right after a correct change "
                        "due to a known reliability rough edge, not a sign "
                        "something else is wrong. Only if none of that "
                        "explains it should you tell the user this specific "
                        "host may be genuinely network-blocked and suggest "
                        "allowlisting it with their admin. See "
                        "references/interpreting.md's network_error section "
